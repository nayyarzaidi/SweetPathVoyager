...
public abstract class DriftGenerator extends AbstractOptionHandler implements InstanceStream {
	...
	public IntOption nAttributes = new IntOption("nAttributes", 'n', "Number of attributes as parents of the class", 2, 1, 1000);
	public IntOption nValuesPerAttribute = new IntOption("nValuesPerAttribute", 'v', "Number of values per attribute", 2, 2, 500);
	public IntOption burnInNInstances = new IntOption("burnInNInstances", 'b', "Number of instances before the start of the drift", 10000, 0, Integer.MAX_VALUE);

	//	public FloatOption driftMagnitudePrior = new FloatOption("driftMagnitudePrior", 'i', "Magnitude of the drift between the starting probability and the one after the drift." + " Magnitude is expressed as the Hellinger distance [0,1]", 0.5, 1e-20, 0.9);
	//	public FloatOption driftMagnitudeConditional = new FloatOption("driftMagnitudeConditional", 'o', "Magnitude of the drift between the starting probability and the one after the drift." + " Magnitude is expressed as the Hellinger distance [0,1]", 0.5, 1e-20, 0.9);

	public FloatOption driftMagnitudePrior = new FloatOption("driftMagnitudePrior", 'i', "Magnitude of the drift between the starting probability and the one after the drift." + " Magnitude is expressed as the Hellinger distance [0,1]", 0.5, 1e-20, 1.0);
	public FloatOption driftMagnitudeConditional = new FloatOption("driftMagnitudeConditional", 'o', "Magnitude of the drift between the starting probability and the one after the drift." + " Magnitude is expressed as the Hellinger distance [0,1]", 0.5, 1e-20, 1.0);

	public FloatOption precisionDriftMagnitude = new FloatOption("epsilon", 'e', "Precision of the drift magnitude for p(x) (how far from the set magnitude is acceptable)", 0.01, 1e-20, 1.0);
	public FlagOption driftConditional = new FlagOption("driftConditional", 'c', "States if the drift should apply to the conditional distribution p(y|x).");
	public FlagOption driftPriors = new FlagOption("driftPriors", 'p', "States if the drift should apply to the prior distribution p(x). ");

	public IntOption seed = new IntOption("seed", 'r', "Seed for random number generator", -1, Integer.MIN_VALUE, Integer.MAX_VALUE);
	...
	public static void generateRandomPxAfterCloseToBefore(double sigma, double[][] base_px, double[][] drift_px, RandomDataGenerator r) {
	...
	public static void generateRandomPyGivenX(double[][] pygx, RandomDataGenerator r) {
		for (int i = 0; i < pygx.length; i++) {
			...
			double classProb = 0.5;
			double nonClassProb = (1 - classProb)/(lineCPT.length - 1);
					
			for (int c = 0; c < lineCPT.length; c++) {
				if (c == chosenClass) {
					lineCPT[c] = classProb;
				} else {
					lineCPT[c] = nonClassProb;
				}
			}
		}
	}
	...
	public static void generateRandomPyGivenX(double[][] pygx, RandomDataGenerator r, double alphaDirichlet) {
		for (int i = 0; i < pygx.length; i++) {
			...
			double sum = 0;
			for (int c = 0; c < lineCPT.length; c++) {
				//alphaDirichlet = r.nextUniform(0.0, 1.0, true);
	...
	public static void generateRandomPx(double[][] px, RandomDataGenerator r, boolean verbose) {
		...
		for (int a = 0; a < px.length; a++) {
			...
			if (verbose)
				System.out.println("p(x_" + a + ")=" + Arrays.toString(px[a]));
		}
	...
	public static double computeMagnitudePX(int nbCombinationsOfValuesPX, double[][] base_px, double[][] drift_px) {
