...
public class SimpleTANGeneratorWithDriftBinaryValues extends DriftGenerator {
...
//			intializeCPTBinary();
		...
		for (int i = 0; i < numDriftAttributes; i++) {
			...
			if (p == 0) {
				...
				intoducePosteriorDrift(p_yxx[p], delta);
			}
		}
	...
	private void intoducePosteriorDrift(double p_yxx[][][], double delta) {
	...
	private void intoducePosteriorDrift(double p_yx[][], double delta) {
	...
	protected void prepareForUseImpl(TaskMonitor monitor, ObjectRepository repository) {
		...
		d_yxx = new int[nc][];
		for (int y = 0; y < nc; y++) {
			d_yxx[y] = new int[nvals];
		}

		/* Initialization */
		//intializeCPT();
		//intializeCPT_Constrained();
		intializeCPTBinary();

		nInstancesGeneratedSoFar = 0L;
	}

	public void intializeCPT_Constrained() {

		int nc = Globals.getNumClasses();
		int n = streamHeader.numAttributes() - 1;
		int nvals = 2;

		p_y[0] = r.nextUniform(delta, 1-delta);
		p_y[1] = 1 - p_y[0];

		for (int y = 0; y < nc; y++) {
			p_yx[y][0] = r.nextUniform(delta, 1 - delta);
			p_yx[y][1] = 1 - p_yx[y][0];
		}
		...
		for (int i = 1; i < n; i++) {
			for (int y = 0; y < nc; y++) {
				for (int x1 = 0; x1 < nvals; x1++) {
					...
					p_yxx[i][y][x1][0] = r.nextUniform(delta, 1 - delta);
	...
	public void intializeCPTBinary() {
		...
		d_yx[0] = r.nextInt(0, 1);
		d_yx[1] = r.nextInt(0, 1);

		d_yxx[0][1] = r.nextInt(0, 1);
		d_yxx[0][0] = r.nextInt(0, 1);
		d_yxx[1][1] = r.nextInt(0, 1);
		d_yxx[1][0] = r.nextInt(0, 1);
	}
