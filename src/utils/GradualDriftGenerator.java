...
public class GradualDriftGenerator extends DriftGenerator{
    ...
    public FloatOption precisionDriftMagnitude = new FloatOption(
            ...
            0.01, 1e-20, 1.0);
    public IntOption driftLength = new IntOption("driftLength", 'l',
            "The number of instances the concept drift will be applied acrros",
            0, 0, 100);
    ...
    private double[][] getIntermidiateProbability(double[][] base_prob, double[][] drift_prob) {
        double[][] inter_prob = new double[base_prob.length][base_prob[0].length];

        for (int i = 0; i < inter_prob.length; i++){
            for (int j = 0; j < inter_prob[i].length; j++){
                double increment_per_unit = (base_prob[i][j] + drift_prob[i][j]) / (driftLength.getValue() + 1);
                long units_to_add = nInstancesGeneratedSoFar - burnInNInstances.getValue();
                inter_prob[i][j] = base_prob[i][j] + increment_per_unit * (double)units_to_add;
            }
        }
        /*
        // normalizing
        for (int v = 0; v < inter_prob[i].length; v++) {
            inter_prob[i][v] /= sum;
    ...
    public InstanceExample nextInstance() {
        double[][] px;
        double[][] pygx;
        if (burnInNInstances.getValue() < nInstancesGeneratedSoFar &&
                nInstancesGeneratedSoFar < burnInNInstances.getValue() + driftLength.getValue()){
            System.out.println("Getting intermediary probability at " + nInstancesGeneratedSoFar + " Instance");
            px = getIntermidiateProbability(pxbd, pxad);
            pygx = getIntermidiateProbability(pygxbd, pygxad);
            System.out.println("Done!");
        }
        else{
            px = (nInstancesGeneratedSoFar <= burnInNInstances
                    .getValue()) ? pxbd : pxad;
            pygx = (nInstancesGeneratedSoFar <= burnInNInstances
    ...
    protected void prepareForUseImpl(TaskMonitor monitor,
        ...
        if (driftPriors.isSet()) {
            ...
            do {
                if (driftMagnitude.getValue() >= 0.2) {
                    generateRandomPx(pxad, r);
                } else if (driftMagnitude.getValue() < 0.2) {
                    generateRandomPxAfterCloseToBefore(driftMagnitude.getValue(), pxbd, pxad, r);
                }
                obtainedMagnitude = computeMagnitudePX(nCombinationsValuesForPX, pxbd, pxad);
            } while (Math.abs(obtainedMagnitude - driftMagnitude.getValue()) > precisionDriftMagnitude
                    .getValue());

            System.out.println("exact magnitude for p(x)="
                    + computeMagnitudePX(nCombinationsValuesForPX, pxbd, pxad) + "\tasked="
                    + driftMagnitude.getValue());
        } else {
            pxad = pxbd;
        }
        ...
        if (driftConditional.isSet()) {
            ...
            int nLinesToChange = (int) Math.round(driftMagnitude.getValue()
